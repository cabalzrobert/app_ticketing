...
export class HeadTicketsComponent {
  ...
  async ngOnInit() {
    ...
    console.log(moment(new Date()).format('MMM DD, yyyy hh:mm A'));
  ...
  forwardingTicket() {
    ...
    dialogRef.afterClosed().subscribe((result: any) => {
      if (result) {
        ...
        this.ticketcomment = [...this.ticketcomment, {
          IsYou: false, 
          isMessage: false, 
          Department: result.forwardDepartment, 
          DisplayName: result.forwardTo, 
          Message: result.forwardRemarks,
          CommentDate: moment(new Date()).format('MMM DD, yyyy hh:mm A')
        }];
      }
    });
  }
